...
const RegistrarDashboard = () => {
    ...
    const approveCourse = async (courseId) => {
        try {
            await axios.put(`${API_URL}/approve-course/${courseId}`);
    ...
    const rejectCourse = async (courseId) => {
        try {
            await axios.put(`${API_URL}/reject-course/${courseId}`);
            fetchPendingCourses(); // Refresh pending courses list
            ...
            <ul>
                {pendingCourses.map((course) => (
                    <li key={course.courseId}>
                        ...
                        <em>Start Date: {new Date(course.courseStartDate).toLocaleDateString()}</em> <br />
                        <em>End Date: {new Date(course.courseEndDate).toLocaleDateString()}</em> <br />
